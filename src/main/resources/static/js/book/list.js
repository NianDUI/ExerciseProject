...
table.on("tool(table)", function (obj) {
    ...
    if (obj.event === "showChapter") {
        ...
        layer.msg("数据整理中请稍等！");
        location.href = getBaseParams("downloadUrl", data.bookid) + "?token=" + handleUrlToken(getToken());
    } else if (obj.event === "download2") {
        layer.msg("数据整理中请稍等！");
        location.href = getBaseParams("downloadUrl2", data.bookid) + "?token=" + handleUrlToken(getToken());
